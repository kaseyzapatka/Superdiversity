 ...
 * File: 2020.08.23 legal permanent residents.do
 ...
 * Data:
	...
	- Tabels 1-12 : https://www.dhs.gov/immigration-statistics/yearbook/2018
	- 2018 thru 1998
	...
	merge 1:1 country using "1996.dta"
		...
		* -----------------------------------------------------
		...
		gen str region = "."
			...
			replace region = "North America" if country == "Saint Kitts-Nevis"
	...
	order country, first 
		...
		* run loop to replace missing with 0s
		foreach v of varlist year-other {
			...
			* combine "refugee_assylee" category with "other" into "other" variable
			* ----------------------------------------------------------
			rename other residual
					
			gen other = refugee_assylee + residual 
						
				* drop extra variables 
				drop residual refugee_assylee
						
				* check that all vars add too total variable
				gen chk = relative + family_sponsored + employment + diversity + other
			...
			* ----------------------------------------------------------
			gen family_based = family_sponsored + relative 
				...
				* check that all vars add too total variable
				gen chk = family_based + employment + diversity + other
