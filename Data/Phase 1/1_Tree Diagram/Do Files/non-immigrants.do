 ...
 * File: 2021.01.28 non-immigrants.do
...
* Load data: 2019: Non-immigrants 
* ===================================
import excel "$rawdir/Non-immigrants/fy2019_table28d.xlsx", clear
	...
	* save file 
	save "$savedir/Non-immigrants/nonimmigrants2019.dta", replace
	...
	* run loop to rename each variable with year suffix
	foreach v of varlist total-unknown {
		...
		* ------------
		...
		replace country = "Congo, Republic" if country == "Congo (Brazzaville)9"
		
		replace country = "Congo, Democratic Republic" if country == "Congo (Kinshasa)10"
	...
	* run loop to rename each variable with year suffix
	foreach v of varlist total-unknown {
		...
		* ------------
		...
		replace country = "Congo, Republic" if country == "Congo (Brazzaville)10"
		
		replace country = "Congo, Democratic Republic" if country == "Congo (Kinshasa)11"
	...
	* run loop to rename each variable with year suffix
	foreach v of varlist total-unknown {
		...
		* ------------
		...
		replace country = "Congo, Republic" if country == "Congo (Brazzaville) 8"
		
		replace country = "Congo, Democratic Republic" if country == "Congo (Kinshasa) 9"
...
use "nonimmigrants2019.dta", clear
		...
		* -------------------
		reshape long total tour_bus_visa tour_bus_other students tempworker diplomats otherclasses unknown, i(country) j(year)
				...
				* ----------------------------------------------------------
				order country, first 
					...
					* run loop to replace missing with 0s
					foreach v of varlist year-unknown {
			...
			* save for collapse command
			save "$savedir/Non-immigrants/nonimmigrants1998_2019.dta", replace
			
			
* Merge legal permanent residents data
* ===================================
do "$dofiledir/non-immigrants_sum.do"
		...
		* -----------------------
				...
				drop if country == "Eswatini (formerly Swaziland)" // drop these and rename Swaziland to this
				...
				replace country = "Unknown or not reported" if country == "Unknown"
...
* =========================================		
	...
	* ----------------------------------------------------------
	...
	order country, first 
		...
		* drop tour_bus_other and tour_bus_visa
		* ----------------------------------------
		drop tour_bus_visa tour_bus_other total
		...
		* ----------------------------------------
		gen other_unknown = otherclasses + unknown
			...
			order other_unknown, last 
			
		
		* generate new total and order variables
		* ---------------------------------------
		gen total = other_unknown + students + tempworker + diplomats
	...
	12. Data is systematically missing for 1997 in legal permanent residents dataset, so we drop data for 1996 and 1997 so 
	    that we have data consistenlty for 20 years: 1998-2019. 
