...
use "$savedir/2020.08.25 Sankey Diagram Data.dta", clear
		...
		* exploration 
		gen id = _n
		bysort ancestry: gen uniqueid = _n
	
		reshape wide language, i(id) j(uniqueid)
		
		order ancestry, after(id)
		
		
		* exploration with count
		gen id = _n
		bysort ancestry: gen uniqueid = _n
	

		reshape wide language count, i(id) j(uniqueid)
	...
	order count, first
		...
		* exploration with count only
		gen id = _n
		bysort ancestry: gen uniqueid = _n
	

		reshape wide count, i(id) j(uniqueid)
