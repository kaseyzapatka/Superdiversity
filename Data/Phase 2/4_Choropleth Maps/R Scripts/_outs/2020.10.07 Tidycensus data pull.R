...
# File: 2020.10.07 Tidycensus data pull.R
# uthor: Kasey Zapatka 
...
# =======================================================
  ...
  remove(list=ls())
    ...
    # ------------------------
    my.packages <- c("tidyverse", "tidycensus", "tmap", "tigris", "sf", "readxl")
...
# =============================================================
    ...
    # new jersey call 
    # ----------------------
     nj <- get_acs(state = "NJ", geography = "tract",  year = 2018,  # specify geographies and year
                   variables = my_vars,
    ...
    # new york call 
    # ----------------------
    ny <- get_acs(state = "NY", geography = "tract",  year = 2018,  # specify geographies and year 
                  variables = my_vars,
                  geometry = TRUE, # specify that we want geometry (shapefiles)
                  output = "wide") # specify that we want the output to be wide
...
# =======================================================
  ...
  # -----------------------
  write_csv(choropleth, "/Users/Dora/Library/Mobile Documents/com~apple~CloudDocs/Projects/Superdiversity/Data/Raw/ACS/tract data for Choropleth Maps.csv")
        ...
        rm(choropleth)
                                ...
                                # using 2020.10.08 Choropleth Map Data Preparation.do
                                # =======================================================
...
# =======================================================    
  ...
  # -----------------------------------------------------
      ...
      traditional <- read_excel("/Users/Dora/Library/Mobile Documents/com~apple~CloudDocs/Projects/Superdiversity/Data/Phase 2/4_Choropleth Maps/Tables/2020.10.08 Choropleth Map - traditional map.xlsx")
  
    glimpse(traditional)
  
      superdiversity <- read_excel("/Users/Dora/Library/Mobile Documents/com~apple~CloudDocs/Projects/Superdiversity/Data/Phase 2/4_Choropleth Maps/Tables/2020.10.08 Choropleth Map - superdiversity map.xlsx") 
  ...
  # -----------------------------------------------------
  map.traditional.missingtracts  <- tm_shape(shapefile.traditional) +
                            tm_polygons() + 
                            tm_view()
  
      tmap_mode("view")
            
  map.traditional.missingtracts
  
  # save traditional.missing map
  tmap_save(map.traditional.missingtracts, "/Users/Dora/Library/Mobile Documents/com~apple~CloudDocs/Projects/Superdiversity/Data/Phase 2/4_Choropleth Maps/Rmaps/Tradional_nonmissingtracts.html", width = 5, height = 7)
  

    
    
  map.superdiversity.missingtracts  <- tm_shape(shapefile.superdiversity) +
                              tm_polygons() + 
                              tm_view()
    
      tmap_mode("view")
    
      map.superdiversity.missingtracts
    
      
      # save superdiversity.missing map
      tmap_save(map.superdiversity.missingtracts, "/Users/Dora/Library/Mobile Documents/com~apple~CloudDocs/Projects/Superdiversity/Data/Phase 2/4_Choropleth Maps/Rmaps/Superdiversity_missingtracts.html", width = 5, height = 7)
  ...
  # -----------------------------------------------------
  shapefile.traditional
    
  shapefile.traditional <- shapefile %>% 
                           filter(st_contains(shapefile.traditional, ., sparse = FALSE))
  
  shapefile.traditional <- shapefile.traditional %>% 
                           filter(st_join(shapefile, ., sparse = FALSE))
  
  shapefile.traditional <- shapefile %>% 
                        st_join(shapefile.traditional, ., sparse = FALSE)
  
 test <- st_join(shapefile.traditional, shapefile)
 
 
  
 map.tracttoPUMA.metro  <- tm_shape(test) +
                           tm_polygons() + 
                           tm_view()
 
 tmap_mode("view")
 
 map.tracttoPUMA.metro
