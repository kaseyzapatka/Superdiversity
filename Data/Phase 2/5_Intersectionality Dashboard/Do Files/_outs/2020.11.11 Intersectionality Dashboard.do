 ...
 * File: 2020.11.11 Intersectionality Dashboard.do
...
fre age 
	...
	******
	compress 
		...
		* ----------------------------
		save "$savedir/2020.11.11 Intersectionality Dashboard Data.dta", replace
		...
		* ----------------------------
		use "$savedir/2020.11.11 Intersectionality Dashboard Data.dta", clear 
			...
			* save
			save "$savedir/2020.11.11 Intersectionality Dashboard Data-hispangrps1.dta", replace
		...
		* ----------------------------
		use "$savedir/2020.11.11 Intersectionality Dashboard Data.dta", clear 
			...
			* save
			save "$savedir/2020.11.11 Intersectionality Dashboard Data-asiangrps1.dta", replace
...
* ============================================================
use "$savedir/2020.11.11 Intersectionality Dashboard-race-ethnicity.dta", clear

	* append hispanic data
	* ---------------------
	append using "$savedir/2020.11.11 Intersectionality Dashboard-hispanic.dta"
	...
	* append asian data
	* ---------------------
	append using "$savedir/2020.11.11 Intersectionality Dashboard-asian.dta"
...
* ============================================================
	...
	* ------------------------------------------
	br rburden rburden_n oburden oburden_n
	
		* gen cburden
		gen cburden = rburden + oburden
		lab var cburden "cost burdened"
		
		* gen cburden_n
		gen cburden_n = rburden_n + oburden_n
		lab var cburden_n "total cost-burdened household"
		
	* Create percent variables for each response
	* ------------------------------------------
	foreach v of varlist coldeg employed lowincome homeownership speakenghome speakforlanghome rburden oburden cburden {
...
* ============================================================
	...
	* drop unnecessary variables
	drop pcoldeg pemployed plowincome phomeownership pspeakenghome pspeakforlanghome prburden poburden pcburden
	...
	drop coldeg_n employed_n lowincome_n homeownership_n speakenghome_n speakforlanghome_n rburden_n oburden_n cburden_n
	
	* drop flags
	* drop *
	
	
	/* create cost burden variable that is average of rent and owner burden 
	* ---------------------------------------------------------------------
	gen cburden = rburden + oburden
	
		* transform to Z-score probability 
		gen p_z_rburden = normprob(z_rburden)
		
		gen p_z_oburden = normprob(z_oburden)
	
	* create cost burden that is weighted average of transformed probabilities 
	gen p_z_cburden = ((p_z_rburden*rburden) + (p_z_oburden*oburden)) / cburden
	
		* convert weighted average probabilities back to z-scores
		gen z_cburden = di invnormal(p_z_cburden)
	
		* drop unnecessary variables 
		drop p_z_cburden p_z_oburden p_z_rburden */
				...
				* ------------------
				...
				lab var cburden "cost-burdened household"
				...
				* ------------------------
				...
				lab var z_cburden "total cost-burdened household, (weighted average)"
				

	
* export to excel 
export excel using "$tabledir/2020.11.11 Intersectionality Dashboard-final.xlsx", sheet("strings", modify) firstrow(variables) keepcellfmt cell(A1)	
