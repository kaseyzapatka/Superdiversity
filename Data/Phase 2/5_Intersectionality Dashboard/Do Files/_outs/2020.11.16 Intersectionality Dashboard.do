 ...
 * File: 2020.11.16 Intersectionality Dashboard.do
...
fre age 
	...
	******
	compress 
		...
		* ----------------------------
		save "$savedir/2020.11.16 Intersectionality Dashboard Data.dta", replace
		...
		* ----------------------------
		use "$savedir/2020.11.16 Intersectionality Dashboard Data.dta", clear 
			...
			* save
			save "$savedir/2020.11.16 Intersectionality Dashboard Data-hispangrps1.dta", replace
		...
		* ----------------------------
		use "$savedir/2020.11.16 Intersectionality Dashboard Data.dta", clear 
			...
			* save
			save "$savedir/2020.11.16 Intersectionality Dashboard Data-asiangrps1.dta", replace
...
* ============================================================
use "$savedir/2020.11.16 Intersectionality Dashboard-race-ethnicity.dta", clear

	* append hispanic data
	* ---------------------
	append using "$savedir/2020.11.16 Intersectionality Dashboard-hispanic.dta"
	...
	* append asian data
	* ---------------------
	append using "$savedir/2020.11.16 Intersectionality Dashboard-asian.dta"
...
* ============================================================
	...
	* ------------------------------------------
	foreach v of varlist coldeg employed hhlowincome homeownership speakenghome speakforlanghome hhrburden hhoburden hhcburden {
...
* ============================================================
	...
	* drop unnecessary variables
	drop pcoldeg pemployed phhlowincome phomeownership pspeakenghome pspeakforlanghome phhrburden phhoburden phhcburden
	...
	drop coldeg_n employed_n hhlowincome_n homeownership_n speakenghome_n speakforlanghome_n hhrburden_n hhoburden_n hhcburden_n
	...
	* ---------------------------------------------------------------------
	gen hhcburden = hhrburden + hhoburden
		...
		* transform to Z-score probability 
		gen p_z_hhrburden = normprob(z_hhrburden)
		
		gen p_z_hhoburden = normprob(z_hhoburden)
	
	* create cost burden that is weighted average of transformed probabilities 
	gen p_z_hhcburden = ((p_z_hhrburden*hhrburden) + (p_z_hhoburden*hhoburden)) / hhcburden
	
		* convert weighted average probabilities back to z-scores
		gen z_hhcburden = di invnormal(p_z_hhcburden)
	
		* drop unnecessary variables 
		drop p_z_hhcburden p_z_hhoburden p_z_hhrburden */
...
* export to excel 
export excel using "$tabledir/2020.11.16 Intersectionality Dashboard-final.xlsx", sheet("strings", modify) firstrow(variables) keepcellfmt cell(A1)	
