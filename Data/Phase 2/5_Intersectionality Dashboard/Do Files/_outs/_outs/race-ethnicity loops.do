	...
	* ------------------------------------------
		...
		* Calculate real percent mean 
		foreach v of varlist coldeg employed lowincome homeownership speakenghome speakforlanghome rburden oburden {
			...
			gen p`v'_mean = p`v' if raceth == "Total - Race-ethnicity" & flag == 1
		...
		* Calculate weighted standard deviation variable 
			...
			* 2.1  normalized = subtract mean, square, and multiply by total N 
			foreach v of varlist coldeg employed lowincome homeownership speakenghome speakforlanghome rburden oburden {
				...
				gen `v'_norm = (`v'_n*(p`v' - p`v'_mean)^2) if flag == 1
				
					format `v'_norm  %15.0fc if flag == 1 // format normalized vairable to remove scientific notation 
		
			} 
			
				* checks
				br raceth coldeg coldeg_n coldeg_tot_n pcoldeg pcoldeg_mean coldeg_norm if flag == 1
				
			* 2.2 column total of normalized variable 
			foreach v of varlist coldeg employed lowincome homeownership speakenghome speakforlanghome rburden oburden {
				
				egen `v'_norm_tot = total(`v'_norm) if flag == 1
				
					format `v'_norm_tot %15.0fc if flag == 1 // format column totoal of normalzed variable to remove scientific notion 
		
			} 
			
				* checks
				br raceth coldeg coldeg_n coldeg_tot_n pcoldeg pcoldeg_mean coldeg_norm coldeg_norm_tot if flag == 1

			* 2.3 take square root of sum of these calculations
			foreach v of varlist coldeg employed lowincome homeownership speakenghome speakforlanghome rburden oburden {
				
				gen `v'_wghtsd = sqrt(`v'_norm_tot/`v'_tot_n) if flag == 1
				...
				* checks
				br raceth coldeg coldeg_n coldeg_tot_n pcoldeg pcoldeg_mean coldeg_norm coldeg_tot coldeg_wghtsd if flag == 1
		...
		* Calculate z-score = (percent- mean percent) / weighted SD for obs greater than 20
		foreach v of varlist coldeg employed lowincome homeownership speakenghome speakforlanghome rburden oburden {
			...
			* checks with and without flags for coldeg
			...
			br raceth employed_n employed_tot_n pemployed pemployed_mean employed_wghtsd employed_norm employed_norm_tot employed_norm_tot employed_norm z_employed if flag == 1
